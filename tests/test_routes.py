...
def test_register(setup):
    rv = setup.post('/register', data={'email': 'test@mail.ru',
                                       'password': 123,
...
def test_register_user_already_exist(db_test_data, setup):
    rv = setup.post('/register', data={'email': 'some@mail.ru',
                                       'password': '123',
                                       'password2': '123'}, follow_redirects=True)
...
def test_login_valid(setup):
    rv = setup.post('/login', data={'email': 'test@mail.ru',
...
def test_login_invalid(setup):
    rv = setup.post('/login', data={'email': 'invalid@mail.ru',
                                    'password': '123'}, follow_redirects=True)
    assert rv.status == '200 OK'
...
def test_not_authenticated_user_create_link(setup):
    rv = setup.post('/links', data={'name': 'test', 'site': 'https://vk.com'}, follow_redirects=True)
    assert rv.status == '200 OK'
    assert b'Please log in to access this page.' in rv.data
